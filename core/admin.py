...
class SiteSettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Ad Tile Settings', {
            'fields': ('tile_ad_every_n', 'enable_tile_click_gating', 'tile_ad_source', 'tile_gating_source'),
        }),
        ('URL Blocking', {
...
@admin.register(AmazonAffiliateProduct)
class AmazonAffiliateProductAdmin(admin.ModelAdmin):
    list_display = ('title_truncated', 'order', 'is_active', 'price_display', 'updated_at')
    list_filter = ('is_active', 'created_at', 'updated_at')
    list_editable = ('order', 'is_active')
    search_fields = ('title', 'affiliate_url', 'price')
    readonly_fields = ('created_at', 'updated_at')
    fieldsets = (
        (None, {
            'fields': ('affiliate_url', 'title', 'image_url', 'price'),
        }),
        ('Display', {
    ...
    def title_truncated(self, obj):
        t = obj.title or obj.affiliate_url or ''
        return (t[:80] + 'â€¦') if len(t) > 80 else t
    title_truncated.short_description = 'Product'

    def price_display(self, obj):
        return obj.price or 'â€”'
