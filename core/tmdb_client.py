...
def get_data_client():
    ...
    Falls back to TMDBClient (live API) when the preferred client fails."""
    settings_obj = SiteSettings.get_settings()
    try:
        if settings_obj.data_source == 'local':
            ...
            if getattr(settings_obj, 'tmdb_db_enabled', True):
                client = TMDBDBClient()
            elif settings_obj.tmdb_db_enable_api_fallback:
                client = TMDBClient()
            else:
                client = LocalDBClient()
        else:
            client = TMDBClient()
        # Quick connectivity probe for DB-backed clients
        if hasattr(client, '_fetch_one'):
            try:
                client._fetch_one('SELECT 1')
            except Exception as db_err:
                print(f"DB probe failed ({db_err}), falling back to TMDB API")
                client = TMDBClient()
        return client
    except Exception as e:
        print(f"Error getting data client: {e}, falling back to TMDB API")
        return TMDBClient()
...
class TMDBDBClient:
    ...
    def discover_movies(self, params=None):
        ...
        try:
            ...
            provider_id = params.get('with_watch_providers')
            watch_region = (params.get('watch_region') or '').upper()
            if provider_id and str(provider_id).isdigit():
                provider_condition = f"id IN (SELECT movie_id FROM {self.movie_provider_table} WHERE provider_id = %s"
                query_params.append(int(provider_id))
                if watch_region:
                    provider_condition += " AND country_code = %s"
                    query_params.append(watch_region)
                conditions.append(provider_condition + ")")
            sort_value = params.get('sort_by', '')
            order_map = {
                'title.asc': 'title ASC', 'title.desc': 'title DESC',
                'primary_release_date.asc': 'release_date ASC', 'primary_release_date.desc': 'release_date DESC',
                'vote_average.asc': 'vote_average ASC', 'vote_average.desc': 'vote_average DESC',
            }
            order_by = order_map.get(sort_value, 'popularity DESC')
    ...
    def discover_series(self, params=None):
        ...
        try:
            ...
            watch_region = (params.get('watch_region') or '').upper()
            if provider_id and str(provider_id).isdigit():
                provider_condition = f"id IN (SELECT tv_id FROM {self.tv_provider_table} WHERE provider_id = %s"
                query_params.append(int(provider_id))
                if watch_region:
                    provider_condition += " AND country_code = %s"
                    query_params.append(watch_region)
            ...
            order_map = {
                ...
                'first_air_date.asc': 'first_air_date ASC', 'first_air_date.desc': 'first_air_date DESC',
                'vote_average.asc': 'vote_average ASC', 'vote_average.desc': 'vote_average DESC',
            }
            order_by = order_map.get(sort_value, 'popularity DESC')
...
class TMDBClient:
    ...
    def _make_request(self, endpoint, params=None):
        ...
        # Aggressive caching for genre/static endpoints
        cache_key = f"tmdb_api:{endpoint}:{hashlib.md5(json.dumps(params, sort_keys=True).encode()).hexdigest()[:12]}"
        if 'genre' in endpoint or endpoint in ('/configuration',):
            cached = cache.get(cache_key)
            if cached:
                return cached

        import time as _time
        keys_to_try = max(len(self.api_keys) * 2, 2)  # Try each key up to 2x
        ...
        for attempt in range(keys_to_try):
            ...
            try:
                # Use a fresh connection for each attempt to avoid stale sockets
                response = self.session.get(url, params=request_params, timeout=(5, 15))
                ...
                # Cache genre/static data for 24 hours
                if 'genre' in endpoint:
                    cache.set(cache_key, result, 86400)
                return result
            except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
                print(f"TMDB connection error on {endpoint} (attempt {attempt+1}): {e}")
                try:
                    self.session.close()
                except Exception:
                    pass
                self.session = requests.Session()
                self.session.headers.update({'Accept': 'application/json'})
                retry_strategy = Retry(total=1, backoff_factor=2, status_forcelist=[429, 500, 502, 503, 504], raise_on_status=False)
                adapter = HTTPAdapter(max_retries=retry_strategy, pool_connections=3, pool_maxsize=5)
                self.session.mount("https://", adapter)
                _time.sleep(2 + attempt * 2)
                continue
            except requests.exceptions.RequestException as e:
                print(f"TMDB API error on {endpoint} (attempt {attempt+1}): {e}")
                _time.sleep(2 + attempt * 2)
                continue

        # Return cached version if available (even stale) as last resort
        if 'genre' in endpoint:
            stale = cache.get(cache_key)
            if stale:
                return stale
        print(f"All TMDB API keys failed for {endpoint}")
    ...
    def get_watch_provider_regions(self):
        return self._make_request('/watch/providers/regions')

    def get_available_watch_providers(self, media_type, watch_region=None):
        endpoint = '/watch/providers/movie' if media_type == 'movie' else '/watch/providers/tv'
        params = {'watch_region': watch_region} if watch_region else None
        return self._make_request(endpoint, params)
...
class LocalDBClient:
    ...
    def _movie_to_dict(self, movie):
        return {
            ...
            'title': movie.title or movie.original_title or '',
            'video': movie.video,
    ...
    def discover_movies(self, params=None):
        try:
            ...
            sort_map = {
                'title.asc': ('title',), 'title.desc': ('-title',),
                'primary_release_date.asc': ('release_date',), 'primary_release_date.desc': ('-release_date',),
                'vote_average.asc': ('vote_average',), 'vote_average.desc': ('-vote_average',),
            }
            query = query.order_by(*sort_map.get(params.get('sort_by'), ('-popularity',)))
    ...
    def discover_series(self, params=None):
        try:
            ...
            sort_map = {
                ...
                'first_air_date.asc': ('first_air_date',), 'first_air_date.desc': ('-first_air_date',),
                'vote_average.asc': ('vote_average',), 'vote_average.desc': ('-vote_average',),
            }
            query = query.order_by(*sort_map.get(params.get('sort_by'), ('-popularity',)))
            page = params.get('page', 1)
