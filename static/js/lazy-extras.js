// ===== Lazy-load TMDB extras (trailers, cast, crew, collection) =====
(function() {
    var mediaType = 'movie';
    var tmdbId = movieId;
    if (!tmdbId) return;
    ...
    .then(function(data) {
        ...
        // Collection
        var colEl = document.getElementById('lazy-collection');
        if (colEl && data.collection) {
            var col = data.collection;
            var parts = data.collection_movies || [];
            var colHtml = '<div class="det-section"><a href="/collection/' + col.id + '/" style="text-decoration:none;color:inherit"><div class="det-collection-wrap"><div class="det-collection-head">';
            if (col.poster_path) colHtml += '<img class="det-collection-poster" src="https://image.tmdb.org/t/p/w185' + col.poster_path + '" alt="' + (col.name||'') + '">';
            colHtml += '<div><div class="det-collection-title">' + (col.name||'') + '</div><div class="det-collection-sub">' + parts.length + ' movies</div></div></div>';
            if (parts.length > 0) {
                colHtml += '<div class="det-collection-list">';
                parts.forEach(function(cm) {
                    colHtml += '<a href="/movies/id/' + cm.id + '/" class="det-collection-item">';
                    if (cm.poster_path) colHtml += '<img src="https://image.tmdb.org/t/p/w185' + cm.poster_path + '" alt="' + (cm.title||'') + '" loading="lazy">';
                    colHtml += '<span class="ci-name">' + (cm.title||'') + '</span>';
                    colHtml += '<span class="ci-year">' + (cm.release_date||'').substring(0,4) + '</span></a>';
                });
                colHtml += '</div>';
            }
            colHtml += '</div></a></div>';
